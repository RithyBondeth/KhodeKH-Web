    ...
    <nav className="fixed top-0 left-0 right-0 z-50 border-b border-border bg-background/80 backdrop-blur-xl">
      <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between gap-4">
        ...
        <div className="hidden md:flex items-center gap-2 shrink-0">
          <ThemeToggle />
          <Link href="/(auth)/login" className="text-sm text-muted-foreground hover:text-foreground px-4 py-2 transition-colors">
            Sign in
          </Link>
          <Link href="/dashboard">
      ...
      {menuOpen && (
        <div className="md:hidden border-t border-border bg-background/95 backdrop-blur-xl animate-scale-in">
          <div className="px-6 py-4 flex flex-col gap-1">
            {NAV_LINKS.map((item) => (
              <Link key={item} href="#" className="py-2.5 text-sm text-muted-foreground hover:text-foreground">
                {item}
              </Link>
            ))}
            <div className="pt-3 border-t border-border flex flex-col gap-2 mt-1">
              <Link href="/(auth)/login" className="py-2 text-sm text-muted-foreground">Sign in</Link>
              <Link href="/dashboard">
